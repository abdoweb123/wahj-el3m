        ...
        <div class="row w-100 justify-content-between">
           <div class="col">
               <h5 style="margin-right:20px; color:#1a1ac3;">
                   <span> الكورسات </span> <span>/</span>  <span> المستندات </span>
               </h5>
           </div>
            ...
            <div class="card card-statistics h-100">
                <div class="card-body">
                    ...
                    @endforeach
                        ...
                        <a href="{{route('pdfs_course_create',$course->id)}}" class="button x-small">
                            إضافة مستند
                        </a>
                    <br><br>
                        ...
                        <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
                            ...
                            @foreach ($pdfs_course as $item)
                                <tr>
                                    ...
                                    <td>@if($item->active == 1)
                                            <a class="btn btn-primary" href="{{route('change_status_pdfs_course',$item->id)}}">نشط</a>
                                        @else
                                            <a class="btn btn-danger" href="{{route('change_status_pdfs_course',$item->id)}}">غير نشط</a>
                                        @endif
                                    </td>
{{--                                    <td>@isset($item->admin->name)  {{ $item->admin->name }} @else _____  @endisset</td>--}}
                                    ...
                                    <td>
                                        <a href="{{route('pdfs_course_show',$item->id)}}" class="process" style="cursor:pointer">
                                ...
                                @include('pages.pdfs_course.delete')


                            @endforeach
                        </table>

                        <div> {{$pdfs_course->links('pagination::bootstrap-4')}}</div>
                    </div>
                </div>
            </div>
        </div>
