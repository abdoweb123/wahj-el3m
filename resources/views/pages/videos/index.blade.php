        ...
        <div class="row w-100 justify-content-between">
           <div class="col">
               <h5 style="margin-right:20px; color:#1a1ac3;">
                   <span> الكورسات </span> <span>/</span>  <span> الفيديوهات </span>
               </h5>
           </div>
            ...
            <div class="card card-statistics h-100">
                <div class="card-body">
                    ...
                    @endforeach
                        ...
                        <a href="{{route('video_create',$course->id)}}" class="button x-small">
                            إضافة فيديو
                        </a>
                    <br><br>
                        ...
                        <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
                            ...
                            @foreach ($videos as $item)
                                <tr>
                                    ...
                                    <td>@if($item->active == 1)
                                            <a class="btn btn-primary" href="{{route('change_status_video',$item->id)}}">نشط</a>
                                        @else
                                            <a class="btn btn-danger" href="{{route('change_status_video',$item->id)}}">غير نشط</a>
                                        @endif
                                    </td>
{{--                                    <td>@isset($item->admin->name)  {{ $item->admin->name }} @else _____  @endisset</td>--}}
                                ...
                                @include('pages.videos.delete')


                            @endforeach
                        </table>

                        <div> {{$videos->links('pagination::bootstrap-4')}}</div>
                    </div>
                </div>
            </div>
        </div>
