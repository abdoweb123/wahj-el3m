        ...
        <div class="row w-100">
            <div class="col">
                <h5 style="margin-right:20px; color:#1a1ac3; text-align: center">
                   <span> كورس </span> <span> {{$course->name}} </span>
                </h5>
            </div>
        </div>
...
{{--                        <form action="{{route('video_store')}}" method="post" enctype="multipart/form-data" name="myForm" onsubmit="return validateForm()">--}}
                        <form action="{{route('intro_video_store')}}" method="post" enctype="multipart/form-data" name="myForm">
                            @csrf
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        let resumable = new Resumable({
            target: '{{ route('upload_intro_video') }}',
